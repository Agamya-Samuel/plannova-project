...
export default function EditVideographyService() {
  ...
  const [formData, setFormData] = useState<VideographyServiceFormData>({
    name: '',
    description: '',
    serviceLocation: {
      address: '',
  ...
  const handleInputChange = (field: string, value: string | number | string[] | undefined) => {
    ...
    if (keys.length > 1) {
      setFormData(prev => {
        const newState: VideographyServiceFormData = { ...prev };
        let current: VideographyServiceFormData | VideographyServiceFormData['serviceLocation'] | VideographyServiceFormData['contact'] = newState;
        for (let i = 0; i < keys.length - 1; i++) {
          current = current[keys[i] as keyof typeof current] as VideographyServiceFormData['serviceLocation'] | VideographyServiceFormData['contact'];
        }
  ...
  const handleAddVideographyType = (type: string) => {
    if (type && !formData.videographyTypes.includes(type)) {
        setFormData(prev => ({ ...prev, videographyTypes: [...prev.videographyTypes, type] }));
    }
  };

  const handleRemoveVideographyType = (type: string) => {
    setFormData(prev => ({ ...prev, videographyTypes: prev.videographyTypes.filter(t => t !== type) }));
  };
  ...
  const isTabCompleted = (tabId: string) => {
    ...
    switch (tabId) {
        ...
        case 'services':
            return formData.videographyTypes.length > 0 && formData.packages.every(p => p.name.trim() && p.price > 0);
        case 'policies':
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const serviceData: VideographyServiceFormData = {
        ...formData,
      ...
      let errorMessage = 'Failed to update videography service';
      if (typeof err === 'object' && err !== null && 'response' in err) {
        const response = (err as { response?: { data?: { error?: string } } }).response;
        if (response?.data?.error) {
          errorMessage = response.data.error;
          ...
          <div className="mb-8">
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  <h1 className="text-4xl font-bold text-gray-900">Edit Videography Service</h1>
                  <p className="text-gray-600 text-lg">Update your videography service profile</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {activeTab === 'basic' && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <Input type="text" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} placeholder="e.g., Premium Wedding Videography" required className="text-black" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Description *</label>
                    <textarea value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} placeholder="Describe your videography service in detail..." rows={4} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-black" required minLength={10} />
                  </div>
                   <div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {videographyTypeOptions.map(type => (
                            <button key={type} type="button" onClick={() => formData.videographyTypes.includes(type) ? handleRemoveVideographyType(type) : handleAddVideographyType(type)}
                                className={`px-4 py-2 rounded-full border text-sm font-medium ${formData.videographyTypes.includes(type) ? 'bg-purple-600 text-white' : 'bg-white text-gray-700'}`}>
                                {type}
                            </button>
                        ))}
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-black">
                    ...
                    <div className="border border-gray-200 rounded-lg p-4">
                      ...
                      <p><strong>Videography Types:</strong> {formData.videographyTypes.join(', ')}</p>
