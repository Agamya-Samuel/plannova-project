...
export default function EditPhotographyService() {
  ...
  const handleAddPhotographyType = (type: string) => {
    if (type && !formData.photographyTypes.includes(type)) {
        setFormData(prev => ({ ...prev, photographyTypes: [...prev.photographyTypes, type] }));
    }
  };

  const handleRemovePhotographyType = (type: string) => {
    setFormData(prev => ({ ...prev, photographyTypes: prev.photographyTypes.filter(t => t !== type) }));
  };
  ...
  const validateCurrentTab = () => {
    ...
    switch (activeTab) {
        ...
        case 'images':
            ...
            break;
        case 'services':
            if (formData.photographyTypes.length === 0) validationErrors.push('At least one photography type is required');
  ...
  const isTabCompleted = (tabId: string) => {
    ...
    switch (tabId) {
        ...
        case 'services':
            return formData.photographyTypes.length > 0 && formData.packages.every(p => p.name.trim() && p.price > 0);
        case 'policies':
          ...
          <div className="mb-8">
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  <h1 className="text-4xl font-bold text-gray-900">Edit Photography Service</h1>
                  <p className="text-gray-600 text-lg">Update your photography service profile</p>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {photographyTypeOptions.map(type => (
                            <button key={type} type="button" onClick={() => formData.photographyTypes.includes(type) ? handleRemovePhotographyType(type) : handleAddPhotographyType(type)}
                                className={`px-4 py-2 rounded-full border text-sm font-medium ${formData.photographyTypes.includes(type) ? 'bg-pink-600 text-white' : 'bg-white text-gray-700'}`}>
                                {type}
                            </button>
                        ))}
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-black">
                    ...
                    <div className="border border-gray-200 rounded-lg p-4">
                      ...
                      <p><strong>Photography Types:</strong> {formData.photographyTypes.join(', ')}</p>
