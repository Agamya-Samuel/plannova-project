...
import { Camera, Plus, X, Upload, ArrowLeft } from 'lucide-react';
...
export default function EditPhotographyService() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (serviceId) {
      ...
      // Redirect to dashboard if no service ID is provided
  ...
  const fetchPhotographyService = async () => {
    try {
      setFetching(true);
      // Check if serviceId is valid before making API call
      if (!serviceId) {
        throw new Error('Service ID is missing');
      }
      
      const response = await apiClient.get(`/photography/${serviceId}`);
      const service: PhotographyService = response.data.data;
      
      // Populate form fields
      setName(service.name);
      setDescription(service.description);
      setAddress(service.serviceLocation.address);
      setCity(service.serviceLocation.city);
      setState(service.serviceLocation.state);
      setPincode(service.serviceLocation.pincode);
      setPhone(service.contact.phone);
      setWhatsapp(service.contact.whatsapp || '');
      setEmail(service.contact.email);
      setBasePrice(service.basePrice.toString());
      setMinGuests(service.minGuests?.toString() || '');
      setCancellationPolicy(service.cancellationPolicy || '');
      setPaymentTerms(service.paymentTerms || '');
      setPhotographyTypes(service.photographyTypes);
      ...
      // Format addons
      setAddons(service.addons.map(addon => ({
        name: addon.name,
      ...
      // Format images
      const formattedImages: VenueImageWithUpload[] = service.images.map((img, index) => ({
        url: img.url,
        alt: img.alt || `Image ${index + 1}`,
        category: 'gallery',
        isPrimary: img.isPrimary || false,
        // Add upload metadata
        key: '', // Will be populated when needed
        name: img.alt || `Image ${index + 1}`,
        type: 'image/jpeg', // Default type
        size: 0, // Size not available from existing images
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Failed to fetch photography service';
      setError(errorMessage);
      toast.error(errorMessage);
      console.error('Error fetching photography service:', err);
      // Redirect to dashboard if there's an error fetching the service
      if (err.response?.status === 404 || err.message === 'Service ID is missing') {
        router.push('/provider/photography');
      }
    } finally {
      setFetching(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.put(`/photography/${serviceId}`, serviceData);
      ...
      setError(errorMessage);
      toast.error(errorMessage);
      console.error('Error updating photography service:', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50 py-8">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-8">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Edit Photography Service</h1>
  ...
  // Add check for missing service ID
  if (!serviceId) {
    return (
      <ProtectedRoute>
        <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50 py-8">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-yellow-700">
              No service ID provided. Please select a service to edit from the dashboard.
            </div>
            <div className="mt-4 text-center">
              <button
                onClick={() => router.push('/provider/photography')}
