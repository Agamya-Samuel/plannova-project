...
export default function DecorationDashboardPage() {
  ...
  const [filteredServices, setFilteredServices] = useState<DecorationService[]>([]);
  const [submitLoading, setSubmitLoading] = useState<string | null>(null);

  // Fetch decoration services for the provider
  useEffect(() => {
    const fetchServices = async () => {
      try {
        setLoading(true);
        const response = await apiClient.get('/decoration/my-services');
        ...
        setError('Failed to load decoration services');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeleteService = async (serviceId: string) => {
    try {
      setLoading(true);
      await apiClient.delete(`/decoration/${serviceId}`);
      ...
      setError('Failed to delete decoration service');
  ...
  const handleSubmitForApproval = async (serviceId: string) => {
    try {
      ...
      await apiClient.patch(`/decoration/${serviceId}/submit-for-approval`);
        ...
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Decoration Services</h1>
                <p className="text-gray-600 mt-2">Manage your decoration packages and services</p>
              </div>
              <Button 
                onClick={() => router.push('/provider/decoration/create')}
            ...
            <>
              ...
              {filteredServices.length === 0 ? (
                <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">No decoration services yet</h3>
                  <p className="text-gray-600 mb-6">
                    Get started by creating your first decoration service
                  </p>
                  <button
                    onClick={() => router.push('/provider/decoration/create')}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredServices.map((service) => (
                      <ServiceCard
                        ...
                        tags={service.decorationTypes}
                        onEdit={(id) => router.push(`/provider/decoration/edit?id=${id}`)}
                        onView={(id) => router.push(`/provider/decoration/view?id=${id}`)}
