...
export default function StaffServicesPage() {
  ...
  const handleReject = async (id: string) => {
    ...
    if (!cfg) return;
    try {
      if (cfg.method === 'post') await apiClient.post(cfg.path, { reason: 'Rejected by staff' });
      else await apiClient.put(cfg.path, { rejectionReason: 'Rejected by staff' });
  ...
  if (!isLoading && !(user?.role === 'STAFF' || user?.role === 'ADMIN')) {
