...
export default function StaffVenueApprovalsPage() {
  const { user } = useAuth();
  ...
  // New function to approve venue edits
  const handleApproveVenueEdit = async (venueId: string) => {
    const confirmed = await sonnerConfirm('Are you sure you want to approve these venue edits?');
    if (!confirmed) {
      return;
    }

    try {
      await apiClient.post(`/venues/staff/${venueId}/approve-edit`);
      toast.success('Venue edits approved successfully!');
      fetchVenues(currentPage, statusFilter, searchTerm);
    } catch (err: unknown) {
      console.error('Error approving venue edits:', err);
      let errorMessage = 'Failed to approve venue edits';
      const apiError = err as ApiError;
      if (apiError.response?.data?.error) {
        errorMessage = apiError.response.data.error;
      }
  ...
  // New function to reject venue edits
  const handleRejectVenueEdit = async (venueId: string) => {
    ...
    try {
      await apiClient.post(`/venues/staff/${venueId}/reject-edit`, { reason: reason.trim() });
      toast.success('Venue edits rejected successfully!');
      fetchVenues(currentPage, statusFilter, searchTerm);
    } catch (err: unknown) {
      console.error('Error rejecting venue edits:', err);
      let errorMessage = 'Failed to reject venue edits';
      const apiError = err as ApiError;
      if (apiError.response?.data?.error) {
        errorMessage = apiError.response.data.error;
      }
  ...
  const handleDeleteVenue = async (venueId: string) => {
    const confirmed = await sonnerConfirm('Are you sure you want to delete this venue? The venue will be moved to the trash where it can be restored later.');
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      toast.success('Venue moved to trash successfully!');
      fetchVenues(currentPage, statusFilter, searchTerm);
    } catch (err: unknown) {
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                <div>
                  <h1 className="text-4xl font-bold text-gray-900 mb-2">
                    Venue Approvals & Edits
                  </h1>
                  <p className="text-gray-600 text-lg">
                    Review and approve provider venue submissions and edits
                  </p>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-12 text-center">
                  ...
                  <p className="text-gray-600 mb-6">
                    {searchTerm && statusFilter !== 'PENDING' && statusFilter !== 'PENDING_EDIT'
                      ...
                      ? "No venues have pending edits for review."
                        ...
                        <div className="md:w-2/3 p-6">
                            ...
                            <div>
                              <div className="flex items-center space-x-2 mb-2">
                                ...
                                <span className={`px-2 py-1 rounded-full text-xs font-medium flex items-center space-x-1 ${getStatusColor(venue.status)}`}>
                                  ...
                                  <span>{venue.status === 'PENDING_EDIT' ? 'Pending Edit' : venue.status}</span>
                                </span>
                              </div>
                              ...
                              <p className="text-sm text-gray-500 mb-2">
                                <strong>Provider:</strong> {venue.providerId?.firstName || 'Unknown'} {venue.providerId?.lastName || 'Provider'} ({venue.providerId?.email || 'No email'})
                              </p>
                              ...
                              <div className="flex items-center justify-center space-x-1 text-gray-600">
                                ...
                                <span className="text-sm">
                                  {venue.status === 'PENDING_EDIT' && venue.pendingEdits?.capacity
                                    ? `${venue.pendingEdits.capacity.min}-${venue.pendingEdits.capacity.max}`
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Button 
                              ...
                              variant="outline" 
                              onClick={() => {
                                // View venue details in same tab
                            ...
                            {venue.status === 'PENDING_EDIT' && (
                              <>
                                <Button
                                  size="sm"
                                  onClick={() => handleApproveVenueEdit(venue._id)}
                                  className="bg-green-600 hover:bg-green-700 text-white flex items-center space-x-1"
                                >
                                  <CheckCircle className="h-4 w-4" />
                                ...
                                <Button
                                  size="sm"
                                  onClick={() => handleRejectVenueEdit(venue._id)}
                                  className="bg-red-600 hover:bg-red-700 text-white flex items-center space-x-1"
                                >
                                  <XCircle className="h-4 w-4" />
