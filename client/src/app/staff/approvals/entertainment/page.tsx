...
interface EntertainmentService {
  ...
  pendingEdits?: Partial<EntertainmentService>;
}

export default function StaffEntertainmentApprovalsPage() {
  const { user } = useAuth();
  ...
  const [selectedService, setSelectedService] = useState<EntertainmentService | null>(null);

  const fetchEntertainmentServices = async (status = 'PENDING', search = '') => {
    try {
      setLoading(true);
      ...
      const response = await apiClient.get<{ data: EntertainmentService[] }>(`/entertainment/staff/pending?${params}`);
      setServices(response.data.data);
      setError('');
    } catch (err) {
      console.error('Error fetching entertainment services:', err);
      setError('Failed to fetch entertainment services');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageClick = (service: EntertainmentService, index: number) => {
  ...
  const handleSearchTermChange = (value: string) => {
    ...
    if (value.trim() === '') {
      fetchEntertainmentServices(statusFilter, '');
      return;
    }
    ...
    const timeout = setTimeout(() => {
      fetchEntertainmentServices(statusFilter, value.trim()).finally(() => {
  ...
  const handleStatusFilterChange = (value: string) => {
    ...
    fetchEntertainmentServices(value, searchTerm);
  };
  ...
  const handleApproveService = async (serviceId: string) => {
    ...
    try {
      ...
      console.error('Error approving entertainment service:', err);
      let errorMessage = 'Failed to approve entertainment service';
  ...
  const handleRejectService = async (serviceId: string) => {
    ...
    try {
      ...
      console.error('Error rejecting entertainment service:', err);
      let errorMessage = 'Failed to reject entertainment service';
      const apiError = err as ApiError;
      if (apiError.response?.data?.error) {
        errorMessage = apiError.response.data.error;
      }
  ...
  const handleApproveServiceEdit = async (serviceId: string) => {
    const confirmed = await sonnerConfirm('Are you sure you want to approve these entertainment service edits?');
    if (!confirmed) {
      return;
    }

    try {
      await apiClient.post(`/entertainment/${serviceId}/approve-edit`);
      toast.success('Entertainment service edits approved successfully!');
      fetchEntertainmentServices(statusFilter, searchTerm);
    } catch (err: unknown) {
      console.error('Error approving entertainment service edits:', err);
      let errorMessage = 'Failed to approve entertainment service edits';
      const apiError = err as ApiError;
      if (apiError.response?.data?.error) {
        errorMessage = apiError.response.data.error;
      }
  ...
  const handleRejectServiceEdit = async (serviceId: string) => {
    ...
    try {
      await apiClient.post(`/entertainment/${serviceId}/reject-edit`, { reason: reason.trim() });
      toast.success('Entertainment service edits rejected successfully!');
      fetchEntertainmentServices(statusFilter, searchTerm);
    } catch (err: unknown) {
      console.error('Error rejecting entertainment service edits:', err);
      let errorMessage = 'Failed to reject entertainment service edits';
      const apiError = err as ApiError;
      if (apiError.response?.data?.error) {
        errorMessage = apiError.response.data.error;
      }
  ...
  const handleDeleteService = async (serviceId: string) => {
    ...
    try {
      ...
      let errorMessage = 'Failed to delete entertainment service';
      const apiError = err as ApiError;
      if (apiError.response?.data?.error) {
        errorMessage = apiError.response.data.error;
      }
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                <div>
                  <h1 className="text-4xl font-bold text-gray-900 mb-2">
                    Entertainment Service Approvals
                  </h1>
                  <p className="text-gray-600 text-lg">
                    Review and approve provider entertainment service submissions
                  </p>
                </div>
              ...
              <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
                ...
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-transparent text-gray-900 placeholder-gray-500"
                    />
                    {searchTerm && (
                      <button
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <select
                    ...
                    className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-transparent text-black"
                  >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    ...
                    fetchEntertainmentServices('PENDING', '');
                  }}
              ...
              {services.length === 0 ? (
                <div className="bg-white rounded-xl shadow-lg p-12 text-center">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">
                    No entertainment services found
                  </h3>
                  <p className="text-gray-600 mb-6">
                    {searchTerm && statusFilter !== 'PENDING' && statusFilter !== 'PENDING_EDIT'
                      ? `No entertainment services found matching "${searchTerm}" with status "${statusFilter}".`
                      : searchTerm
                      ? `No entertainment services found matching "${searchTerm}".`
                      : statusFilter !== 'PENDING' && statusFilter !== 'PENDING_EDIT'
                      ? `No entertainment services found with status "${statusFilter}".`
                      : statusFilter === 'PENDING_EDIT'
                      ? "No entertainment services have pending edits for review."
                      : "No entertainment services are currently pending approval."}
                  </p>
                </div>
              ) : (
                        ...
                        <div className="md:w-2/3 p-6">
                            ...
                            <div className="text-center">
                              <div className="flex items-center justify-center space-x-1 text-gray-600">
                                ...
                                <span className="text-sm">
                                  {service.status === 'PENDING_EDIT' && service.pendingEdits?.entertainmentTypes
                                    ? service.pendingEdits.entertainmentTypes.length
                                    : service.entertainmentTypes.length}
                                </span>
                              </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Button 
                              ...
                              variant="outline" 
                              onClick={() => {
                                router.push(`/staff/approvals/entertainment/view?id=${service._id}`);
                              }}
